...
def main():
    ...
    train_groups = train.groupby(['Drug', 'Mtx']).groups
    print train_groups.keys()
    test_groups = test.groupby(['Drug', 'Mtx']).groups
    print test_groups.keys()
    trained_methods = {}
    ...
    for group in train_groups:
        if group[0] is np.nan or group[1]== -1:
            continue
        y = np.array(train["Response.deltaDAS"])[train_groups[group]]
        x = np.array(train[selected_columns])[train_groups[group],:]
        ...
        trained_methods[group] = comp.process(x, y, None, train_cv=10,
                              tr_scoring=make_scorer(scorer, greater_is_better=True), n_jobs=8)


    final_results = {}
    for group in test_groups:
        x_test = np.array(test[selected_columns])[test_groups[group],:]
        ids = np.array(test["ID"])[test_groups[group]]
        if group in train_groups:
            for method in trained_methods[group]:
                predictions ={method: trained_methods[group][method].predict(x_test)}
            ...
            for treatment in ['adalimumab','infliximab']:
                train_group = (treatment, group[1])
                for method in trained_methods[train_group]:
                    predictions ={method: trained_methods[train_group][method].predict(x_test)}
                    if method in final_results:
                        final_results[method] = update_results(ids, predictions, final_results[method])
                    else:
                        final_results[method] = update_results(ids, predictions, {})
...
if __name__ == "__main__":
    ...
    for model in results:
        write_results(os.path.join(OUTPUT,'results_sepTreat_sepmtx_%s.csv'%model),results[model],ids)