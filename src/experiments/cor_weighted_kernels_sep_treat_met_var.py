...
def main():
    ...
    selected_columns = ["baselineDAS", "Age", "Gender", "Mtx"]
    ...
    for drug in train_groups:
        ...
        x = np.array(train[selected_columns].replace(np.nan,0))[train_groups[drug],:]
        ...
        params_grid = {
            'sumSVR': {
                ...
                'kernel_functions':[[cosine_similarity, cosine_similarity, DiracKernel, DiracKernel]],
                'w': list(product(range(1,6), repeat=4))
            }
        }
    ...
    for drug in test_groups:
        x_test = np.array(test[selected_columns].replace(np.nan,0))[test_groups[drug],:]
...
if __name__ == "__main__":
    ...
    for model in results:
        write_results(os.path.join(OUTPUT,'results_separate_treatment_met_var%s.csv'%model),results[model],ids)